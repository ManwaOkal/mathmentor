-- Quick script to apply migration 013 if it hasn't been run
...
-- Enable RLS on activity_documents if not already enabled
ALTER TABLE public.activity_documents ENABLE ROW LEVEL SECURITY;

-- RLS Policies for activity_documents
DROP POLICY IF EXISTS "Teachers can manage activity documents" ON public.activity_documents;
CREATE POLICY "Teachers can manage activity documents"
    ON public.activity_documents FOR ALL
    USING (auth.uid() = teacher_id);

DROP POLICY IF EXISTS "Students can view activity documents in enrolled classrooms" ON public.activity_documents;
